	...
	for (int k=0;k<nk;k++)
        for (int j=0;j<nj;j++)
        	for (int i=0;i<ni;i++)
            {
				...
				else if (k==nk-1) {A(u,u)=idz;A(u,u-ni*nj)=-idz;}               
                else {
                	//standard internal stencil
					int uy_left = (j==0)?world.U(i,nj-2,k):u-ni;	//periodic boundary
					int uy_right = (j==nj-1)?world.U(i,1,k):u+ni;	//periodic boundary
					A(u,u-ni*nj) = idz2;
                	A(u,uy_left) = idy2;
                	A(u,u-1) = idx2;
	...
	for (int i=0;i<world.ni;i++)
		for (int j=0;j<world.nj;j++)
			for (int k=0;k<world.nk;k++)
			{
				...
				/*y component, periodic boundary*/
				if (j==0)
					ef[i][j][k][1] = -(phi[i][1][k] - phi[i][world.nj-2][k])/(2*dy);
