...
void Species::advance()
{
	...
	for (Particle &part: particles)
	{
		...
		/*did the particle leave through a symmetric boundary?*/
		if (part.pos[0]<x0[0] || part.pos[1]<x0[1]) {
			if (part.pos[0]<x0[0]) {
				part.pos[0] = 2*x0[0] - part.pos[0];
				part.vel[0] *= -1;	//flip velocity
			}
			if (part.pos[1]<x0[1]) {
				part.pos[1] = 2*x0[1] - part.pos[1];
				part.vel[1] *= -1;	//flip velocity
			}
		}
		...
		part.dt = 0;	//this particle finished the entire step
	}
