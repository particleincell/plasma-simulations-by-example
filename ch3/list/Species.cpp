...
void Species::advance()
{
	...
	/*perform a particle removal step*/
	std::list<Particle>::iterator it = particles.begin();
	while(it!=particles.end())
	{
		Particle &part = *it;			//dereference the iterator
		if (part.mpw>0) {++it;continue;}	//ignore live particles
		it = particles.erase(it);	//delete the item and get a new iterator to the current position
	}
}
