...
void World::computeChargeDensity(vector<unique_ptr<Species>> &species)
{
	rho = 0;
	for (auto &sp:species)
	{
...
bool World::steadyState(vector<unique_ptr<Species>> &species) {
	...
	for (auto &sp:species)
	{
		tot_mass += sp->getMass();	//number of real molecules
