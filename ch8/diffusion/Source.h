...
public:
	ColdBeamSource(Species &species, World &world, double v_drift, double den) :
		sp{static_cast<KineticSpecies&>(species)}, world{world}, v_drift{v_drift}, den{den} {}

	//generates particles
	void sample();

protected:
	KineticSpecies &sp;	//reference to the injected species
...
public:
	WarmBeamSource(Species &species, World &world, double v_drift, double den, double T) :
		sp{static_cast<KineticSpecies&>(species)}, world{world}, v_drift{v_drift}, den{den}, T{T} {}
