...
struct vec3 {
	...
	T friend mag(const vec3<T> &o) {return sqrt(o(0)*o(0)+o(1)*o(1)+o(2)*o(2));}
	T friend dot(const vec3<T> &a, const vec3<T> &b) {return a(0)*b(0)+a(1)*b(1)+a(2)*b(2);}
...
public:
	...
	friend Field_ operator+(const Field_ &f, const Field_ &g) {
		Field_ r(f.ni,f.nj,f.nk);
		for (int i=0;i<f.ni;i++)
			for (int j=0;j<f.nj;j++)
				for (int k=0;k<f.nk;k++)
					r[i][j][k] = f(i,j,k)+g(i,j,k);
		return r;
	}

	/*performs element by element scaling*/
