...
bool PotentialSolver::solveNRPCG()
{
	...
	for(int it=0;it<NR_MAX_IT;it++)
	{
		...
		if (!solvePCGLinear(J,y,F))
			solveGSLinear(J,y,F,max_solver_it,tolerance);
...
{
	computeNegGradient(world.ef,vec::deflate(world.phi),world);
}

//computes -grad(phi)
void PotentialSolver::computeNegGradient(Field3 &g, const dvector &phi, const World &world)
{
	double3 dh = world.getDh();
	double dx = dh[0];
	double dy = dh[1];
	...
	for (int i=0;i<world.ni;i++)
		for (int j=0;j<world.nj;j++)
			for (int k=0;k<world.nk;k++)
			{
				...
				/*x component*/
				if (i==0)
					g[i][j][k][0] = -(-3*phi[u]+4*phi[u+1]-phi[u+2])/(2*dx);	/*forward*/
				else if (i==world.ni-1)
					g[i][j][k][0] = -(phi[u-2]-4*phi[u-1]+3*phi[u])/(2*dx);	/*backward*/
				else
					g[i][j][k][0] = -(phi[u+1] - phi[u-1])/(2*dx);	/*central*/

				/*y component*/
				if (j==0)
					g[i][j][k][1] = -(-3*phi[u] + 4*phi[u+ni]-phi[u+2*ni])/(2*dy);
				else if (j==world.nj-1)
					g[i][j][k][1] = -(phi[u-2*ni] - 4*phi[u-ni] + 3*phi[u])/(2*dy);
				else
					g[i][j][k][1] = -(phi[u+ni] - phi[u-ni])/(2*dy);

				/*z component*/
				if (k==0)
					g[i][j][k][2] = -(-3*phi[u] + 4*phi[u+ni*nj]-phi[u+2*ni*nj])/(2*dz);
				else if (k==world.nk-1)
					g[i][j][k][2] = -(phi[u-2*ni*nj] - 4*phi[u-ni*nj]+3*phi[u])/(2*dz);
				else
					g[i][j][k][2] = -(phi[u+ni*nj] - phi[u-ni*nj])/(2*dz);
			}
}
